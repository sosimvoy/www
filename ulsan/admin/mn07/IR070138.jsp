...
<%
  ...
  String cpage = request.getParameter("cpage");	// 현재 페이지 번호 받기
    ...
    if(saveFile.equals("1")){
        ...
        response.setHeader("Content-Disposition", "inline; filename=세출분기표(회계별)_" +new java.sql.Date(System.currentTimeMillis()) + ".xls");   
...
<form name="sForm" method="post" action="IR070138.etax">
<input type="hidden" name="cmd" value="dayReport28">      
  ...
  <tr>
    <td>
      <table width="960" border="0" cellspacing="0" cellpadding="1" class="report">
        <tr height="185"> 
          <td colspan="7"><p><b><u>
            <%=TextHandler.formatDate(TextHandler.replace(request.getParameter("acc_year"),"-",""),"yyyyMMdd","yyyy")%>년 <%=quarter%>/4분기 세출 분기표
            </u></b></p>
          </td>
