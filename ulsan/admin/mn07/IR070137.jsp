...
* 프로그램작성자	: (주)미르이즈 
* 프로그램작성일	: 2010-09-09
* 프로그램내용		: 일계/보고서 > 보고서조회 > 세입분기표(일반회계세목별)
*************************************************************************/
%>
...
<%
  CommonEntity endState = (CommonEntity)request.getAttribute("page.mn07.endState");
    ...
    if(saveFile.equals("1")){
        ...
        response.setHeader("Content-Disposition", "inline; filename=세입분기표(일반회계세목별)_" +new java.sql.Date(System.currentTimeMillis()) + ".xls");   
...
<form name="sForm" method="post" action="IR070137.etax">
<input type="hidden" name="cmd" value="dayReport27">      
    ...
    <td>
      <table width="960" border="0" cellspacing="0" cellpadding="1" class="report">
        <tr height="110"> 
          <td>
            <p><b><u>
              <%=TextHandler.formatDate(TextHandler.replace(request.getParameter("acc_date"),"-",""),"yyyyMMdd","yyyy")%>년 <%=quarter%>/4분기 세입 분기표
            </u></b></p>
          </td>
          <td colspan="2" width="200" align="right" valign="top" rowspan="2"> 
            <%
