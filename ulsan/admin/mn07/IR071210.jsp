...
<%
  ...
  List<CommonEntity> nongHyupList  = (List<CommonEntity>)request.getAttribute("page.mn07.nongHyupList");  //광역시세세입일계표

	List<CommonEntity> nongHyupGwaList  = (List<CommonEntity>)request.getAttribute("page.mn07.nongHyupGwaList");  //과오납환부일계표
  
  String ago_acc_date  = (String)request.getAttribute("page.mn07.ago_acc_date");    // 전영업일 받아오기
	String data_type =  (String)request.getAttribute("page.mn07.data_type");
...
<html>
  <head>
    ...
    <script language="javascript" src="../js/calendar.js"></script>	
      ...
      function init() {
        ...
        } else if (data_type == "G7") {
          form.trans_gubun.value = "162";
        } else if (data_type == "G8") {
          form.trans_gubun.value = "163";
        } else if (data_type == "G9") {
        ...
        if(form.upfile.value.substring(idx1+1, idx2) != "xls"){
          ...
          return;
				...
				if (confirm("첨부한 엑셀파일("+form.upfile.value+")과 자료구분("+data_type+")과 일치합니까?")) {
					form.action = "IR071211.etax?cmd=nongHyupTmpUp07&uploadDir=excel&flag=Y&acc_date="+form.acc_date.value;
        ...
        if(form.upfile.value.substring(idx1+1, idx2) != "xls"){
          ...
          return;
				...
				if (confirm("첨부한 엑셀파일("+form.upfile.value+")과 자료구분("+data_type+")과 일치합니까?")) {
				  ...
				  form.action = "IR071210.etax?cmd=nongHyupReg07&uploadDir=excel&flag=N&trans_gubun="+ form.trans_gubun.value +"&acc_date=" +form.acc_date.value;
          form.target = "_self";
          eSubmit(form);
				}        
      }          
  ...
  <body topmargin="0" leftmargin="0">
	...
	<input type="hidden" name="cmd" value="nongHyupDel07">
  ...
  <form name="sForm" method="post" enctype="multipart/form-data"  action="IR071210.etax">
	<input type="hidden" name="cmd" value="nongHyupList07">
						          ...
						          <img src="../img/btn_calendar.gif" style="cursor:hand" onclick="Calendar(this,'sForm','acc_date')" align="absmiddle">
									    ...
									    자료구분
											<select name="data_type" iValue="<%=request.getParameter("data_type")%>">
												<option value="G6">광역시세 세입일계표</option>
												<option value="G7">과오납 환부일계표(세입)</option>
                        <option value="G8">과오납 환부일계표(과오납)</option>
                        <option value="G9">차량등록사업소(자체OCR수납분)</option>
                        <option value="G9">차량등록사업소(금결원카드수납분)</option>
                        <option value="G9">차량등록사업소(금결원일반수납분)</option>
											</select>
											<br><br>
											<span style="width:50"></span>
										  <img src="../img/board_icon1.gif" width="8" height="8" align="absmiddle"> 
