...
* 프로그램명        : IR050210Update1CMD.java
* 프로그램작성자    :
* 프로그램작성일    :
* 프로그램내용      : 자금배정 > 자금배정승인내역조회/배정처리 - 배정취소
***********************************************************************/
...
public class IR050210Update1CMD extends BaseCommand {

  private static Logger logger = Logger.getLogger(IR050210Update1CMD.class); // log4j 설정
  ...
  public void execute(HttpServletRequest request, HttpServletResponse response, Connection conn) throws SQLException{ 
    ...
    logger.info("paramInfo : " + paramInfo);
		...
		for (int i=0; i<allotChk.length; i++)	{
			...
			if ("Y".equals(allot_yn[y])) {
				if (IR050210.allotDelete(conn, seq_list[y], paramInfo) < 1)	{
    ...
    request.setAttribute("page.mn05.allotList", allotList);
		...
		request.setAttribute("page.mn05.SucMsg", "배정취소처리되었습니다.");
  }
}