...
public class IR050210TransCMD extends BaseCommand {

    private static Logger logger = Logger.getLogger(IR050210TransCMD.class); // log4j 설정
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response, Connection conn) throws SQLException{ 
		...
		CommonEntity magamInfo = IR050210.getMagamInfo(conn, paramInfo);
		if ("Y".equals(magamInfo.getString("M210_WORKENDSTATE")) ) {
		    ...
		    CommonEntity outAcct = IR050210.getOutAcctInfo(conn, paramInfo); //출금계좌정보
		    ...
		    if ("".equals(outAcct.getString("M300_ACCOUNTNO")) || outAcct.getString("M300_ACCOUNTNO") == null) {
		        ...
		        for (int i=0; i<allotChk.length; i++) {
		            ...
		            if ("Y".equals(allot_yn[y])) {
		                total ++;
		                CommonEntity inAcct = IR050210.getInAcctInfo(conn, paramInfo, seq_list[y]);
		                if (!"".equals(inAcct.getString("M300_ACCOUNTNO"))) { //입금계좌 존재할 때만
		                    ...
		                    IR050210.updateAllotPms(conn, paramInfo, seq_list[y]);
		                    cnt ++;
		                }
		            }
		        }
