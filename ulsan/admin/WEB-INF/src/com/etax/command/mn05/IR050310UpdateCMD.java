...
public class IR050310UpdateCMD extends BaseCommand {

  private static Logger logger = Logger.getLogger(IR050310UpdateCMD.class); // log4j 설정
  ...
  public void execute(HttpServletRequest request, HttpServletResponse response, Connection conn) throws SQLException{ 
    ...
    logger.info("paramInfo : " + paramInfo);
		...
		for (int i=0; i<allotChk.length; i++)	{
			...
			if ("Y".equals(allot_yn[y])) {
				if (IR050310.updateReAllotStat(conn, seq_list[y], paramInfo) < 1)	{
    ...
    request.setAttribute("page.mn05.reAllotList", reAllotList);
		...
		request.setAttribute("page.mn05.SucMsg", "재배정처리되었습니다.");
  }
}