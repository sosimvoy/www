...
* 프로그램명     : IR070139SelectCMD.java
* 프로그램작성자 : (주)미르이즈
* 프로그램작성일 : 2010-09-13
* 프로그램내용   : 일계보고서 > 보고서 조회 > 지급증명서
****************************************************************/
...
import com.etax.db.mn07.IR070139;
import com.etax.db.common.ReportDAO;
import com.etax.entity.CommonEntity;

public class IR070139SelectCMD extends BaseCommand {
	private static Logger logger = Logger.getLogger(IR070139SelectCMD.class);	// log4j 설정
	...
	/* 조회할 페이지 범위 구하기 */
	    ...
	    paramInfo.setValue("acc_year",	acc_year);		
		  paramInfo.setValue("acc_date",	acc_date);	// 회계일자
      ...
      paramInfo.setValue("part_code",	request.getParameter("part_code"));		// 부서코드
		...
		// 보고서
  	    List<CommonEntity> reportList = IR070139.getReportList(conn, paramInfo);
		request.setAttribute("page.mn07.reportList", reportList);

		/* 총 카운트 조회 */
		CommonEntity rowData = IR070139.getTotalCount(conn, paramInfo);
